...
export default function AssetUnitsPage() {
  return (
    <SiteShell title="Inventory units">
      <Head>
        <title>Inventory units | Print farm</title>
      </Head>
      <AssetClient mode="units" />
    </SiteShell>
  );
}
...
export async function getServerSideProps(context) {
  ...
  if (!actor) {
    return {
      redirect: {
        destination: "/auth/signin?callbackUrl=%2Fadmin%2Fassets%2Funits",
        permanent: false,
      },
    };
  }
