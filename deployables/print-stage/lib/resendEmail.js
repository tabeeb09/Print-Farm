...
export async function sendDailyQuotaAlertEmail({ to, status }) {
  return sendResendEmail({
    ...
    text: [
      `The print farm account recovery email limit has been reached for ${status.day}.`,
      "",
      `Configured daily hard limit: ${status.limit}`,
      `Recorded outbound email events: ${status.count}`,
      "",
      "Further account recovery emails are blocked until the next UTC day.",
      "",
    ...
    html: [
      `<p>The print farm account recovery email limit has been reached for <strong>${status.day}</strong>.</p>`,
      `<p><strong>Configured daily hard limit:</strong> ${status.limit}<br/>`,
      `<strong>Recorded outbound email events:</strong> ${status.count}</p>`,
      "<p>Further account recovery emails are blocked until the next UTC day.</p>",
