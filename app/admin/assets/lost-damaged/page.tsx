...
export default async function LostDamagedAssetsPage() {
  ...
  if (!hasAssetAdminRole(roles)) {
    return (
      <section className="assetPage panel">
        ...
        <p>You need the owner or asset_admin role to manage lost and damaged equipment.</p>
        <p>Current roles: {roles.length ? roles.join(", ") : "none detected"}</p>
        <PrivilegeRequestButton resource="assets/lost-damaged" requestedRole="asset_admin" />
      </section>
    );
  }

  return <AssetClient mode="lost-damaged" />;
}
