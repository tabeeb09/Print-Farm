  ...
  if (!hasAssetAdminRole(roles)) {
    return (
      <section className="assetPage panel">
        ...
        <p>You need the owner or asset_admin role to manage catalogue items.</p>
        <p>Current roles: {roles.length ? roles.join(", ") : "none detected"}</p>
        <PrivilegeRequestButton resource="assets/catalogue" requestedRole="asset_admin" />
      </section>
    );
  }

  return <AssetClient mode="catalogue" />;
}
